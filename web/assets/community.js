...
export function syncCommunityUi() {
  ...
  if (owned) owned.innerHTML = contributionRows();
  ...
  if (button && button.textContent !== t("busy")) button.disabled = !session;
...
export async function handleForm(form, rerender) {
  ...
  try {
    ...
    rerender();
    toast(
      t(kind === "contributions" ? "contributionSuccess" : "submitSuccess"),
    );
  } catch (error) {
...
async function confirmWithdrawal() {
  ...
  dialog.innerHTML = `<form method="dialog"><div class="dialog-icon">${icon("info")}</div><h2>${t("confirmation")}</h2><p>${t("withdrawConfirm")}</p><p class="small muted">${t("faqWithdrawA")}</p><div class="dialog-actions"><button class="button button-outline" value="cancel" autofocus>${t("cancel")}</button><button class="button button-danger" value="confirm">${t("withdraw")}</button></div></form>`;
  dialog.returnValue = "";
  dialog.showModal();
