...
import { esc, link, icon, date, number, safeUrl, image, avatar, sourceHref, pastEvent, textMatch, empty, platformName } from './utils.js';
...
function originalPost(post, catalog, following) {
  ...
  return `<section class="event-reference feed-content" data-event-post="${esc(post.id)}"><h3 class="event-reference-label">${l(post.contentKind === 'website_snapshot' ? 'snapshot' : 'reference')}</h3><div class="event-author">${link(source ? sourceHref(source) : post.sourceUrl, avatar({ ...source, name, avatar: source?.avatar || post.avatar }) + `<strong>${esc(name)}</strong>`, 'event-author-link')}<span>${esc(platformName(post.platform))}${post.publishedAt ? ` · ${esc(date(post.publishedAt))}` : ''}</span></div>${post.text ? `<p class="feed-text ${expanded ? 'is-expanded' : ''}">${esc(post.text)}</p>${expanded ? '' : `<button type="button" class="feed-expand" data-expand-post aria-expanded="false">${t('readMore')}</button>`}` : ''}<div class="event-reference-actions">${source ? `<button type="button" class="feed-action ${followed ? 'is-following' : ''}" data-follow="${esc(source.id)}" aria-pressed="${!!followed}" aria-label="${esc(t(followed ? 'unfollow' : 'follow') + ' ' + name)}">${icon(followed ? 'check' : 'heart')}<span>${t(followed ? 'following' : 'follow')}</span></button>` : ''}${link(post.url, t('original'), 'text-link')}</div></section>`;
}
