...
const labels = {
  'zh-Hant': {
    ...
    academicYear: '學年度（民國年）', yearNote: '臺灣比賽學年度保留來源的民國年標示。',
  ...
  en: {
    ...
    academicYear: 'Academic year (ROC)', yearNote: 'Taiwan competition years retain the ROC year notation used by the source.',
  ...
  ja: {
    ...
    academicYear: '学年度（民国年）', yearNote: '台湾のコンクール年度は、情報源の民国年表記を保持しています。',
  ...
  ko: {
    ...
    academicYear: '학년도(민국년)', yearNote: '대만 콩쿠르 학년도는 출처의 민국년 표기를 유지합니다.',
...
export function scoreFacets(rows, state = {}, key) {
  ...
  return [...counts].sort(([a], [b]) => key === 'year' ? (Number(b) || 0) - (Number(a) || 0) : collator.compare(a, b)).map(([value, count]) => ({ value, count }));
}
...
function select(key, label, allLabel, rows, state) {
  const valueLabel = value => key === 'country' ? countryName(value) : key === 'instrument' ? instrumentation(value) : value;
