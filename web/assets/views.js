...
export function sortSources(rows, sort = "name", descending = false, catalog = {}) {
  ...
  const value = s => sort === "posts" ? counts.get(s.id) || 0 : sort === "updated" ? Date.parse(s.updatedAt) || 0 : sort === "country" ? countryName(s.countryCode, s.country) : sort === "type" ? sourceType(s.type) : s.name;
...
export function sourceCard(s, following = new Set(), catalog) {
  ...
  return `<article class="source-card src-row"><h3 class="src-c-name">${link(sourceHref(s), avatar(s) + `<span class="src-name">${esc(s.name)}</span><span class="src-country-mobile" title="${esc(countryName(s.countryCode, s.country))}">${esc(s.countryCode || countryName(s.countryCode, s.country))}</span>`, "source-avatar-link", `title="${esc(s.name)}"`)}</h3><span class="src-country">${esc(countryName(s.countryCode, s.country))}</span><div class="src-c-chips"><span class="tag">${esc(sourceType(s.type))}</span></div><div class="src-links">${(s.links || []).map(l => link(l.url, esc(l.label || t("original")), "src-link")).join("")}</div><span class="src-count" title="${esc(t("posts"))}">${count === undefined ? "—" : number(count)}</span><time class="src-upd" datetime="${esc(s.updatedAt || "")}">${s.updatedAt ? esc(date(s.updatedAt, { year: undefined })) : "—"}</time><button class="follow-button src-c-follow ${followed ? "is-following" : ""}" data-follow="${esc(s.id)}" aria-pressed="${followed}" aria-label="${esc(t(followed ? "unfollow" : "follow") + " " + s.name)}">${icon(followed ? "check" : "heart")}<span>${t(followed ? "following" : "follow")}</span></button></article>`;
}
