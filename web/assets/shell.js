...
export function navigation(current = location.pathname, routes = {}) {
  const primary = [['/', 'discover'], ['/post/', 'posts'], ['/events/', 'events'], ['/source/', 'sources']];
  const extras = Object.entries(routes).filter(([url]) => !primary.some(([p]) => p === url));
  const brand = getLocale() === 'zh-Hant' ? '口琴觀測站' : 'Harmonica Observatory';
  return `<a class="skip-link" href="#main">${t('skip')}</a><header class="site-header"><button class="mobile-header-action mobile-filter" data-shell-action="filter" aria-label="${t('filter')}">${glyph('filter')}</button><a class="brand" href="/" aria-label="${esc(brand)}"><span class="brand-mark">${glyph('harmonica')}</span><span class="brand-name">${brand}<small>${getLocale() === 'zh-Hant' ? 'Harmonica Observatory' : '口琴觀測站'}</small></span></a><button class="mobile-header-action mobile-search" data-shell-action="search" aria-label="${t('search')}">${glyph('search')}</button><nav class="site-nav" aria-label="${t('navigation')}">${primary.map(([url, key]) => navLink(url, key, current)).join('')}${extras.filter(([url]) => ['/scores/', '/feeds/', '/contribute/', '/submit/'].includes(url)).map(([url, key]) => navLink(url, key, current, 'nav-desktop-extra')).join('')}<details class="nav-more"><summary class="nav-item" title="${text('more')}">${glyph('menu')}<span class="nav-label">${text('more')}</span></summary><div class="nav-more-menu"><div data-shell-panel="main">${extras.map(([url, key]) => `<a href="${url}" ${['/scores/', '/feeds/', '/contribute/', '/submit/'].includes(url) ? 'class="nav-mobile-extra"' : ''}>${glyph(navIcons[key])}<span>${t(key)}</span></a>`).join('')}<button class="appear-open" data-shell-action="appearance">${glyph('palette')}<span>${text('appearance')}</span>${glyph('chevron')}</button><label class="language-picker" for="language-select">${t('language')}<select id="language-select">${locales.map((locale) => `<option value="${locale}" ${locale === getLocale() ? 'selected' : ''}>${languageNames[locale]}</option>`).join('')}</select></label></div><div data-shell-panel="appearance" hidden><div class="appear-head"><button class="appear-back" data-shell-action="back" aria-label="${text('back')}">←</button><strong>${text('appearance')}</strong></div><div class="appear-seg">${['light', 'dark', 'system'].map((theme) => `<button data-theme-choice="${theme}" aria-pressed="${theme === storedTheme()}">${text(theme)}</button>`).join('')}</div></div></div></details></nav></header>`;
}
export function footer() {
  return `<footer class="site-footer"><div class="footer-links">${[['/about/', 'about'], ['/privacy/', 'privacy'], ['/status/', 'status'], ['/feeds/', 'feeds'], ['/submit/', 'submit'], ['/scores/sources/', 'scoreSources']].map(([url, key]) => link(url, t(key))).join('')}</div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Harmonica Observatory · 口琴觀測站</span>${link('https://github.com/skyhong2002/chumei', t('credit'))}${link('/api/v1/catalog', t('dataLink'))}</div></footer>`;
}
