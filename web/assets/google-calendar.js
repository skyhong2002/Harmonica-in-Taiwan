...
export function googleCalendarView(catalog) {
  ...
  const allUrl = googleCalendarUrl(calendars);
  return `<div class="google-calendar" data-google-calendar><div class="google-calendar-toolbar"><div class="google-calendar-filters" role="group" aria-label="${esc(label('filter'))}">${calendars.map(calendar=>`<label><input type="checkbox" data-google-calendar-source data-calendar-source data-calendar-id="${esc(calendar.id)}" data-calendar-key="${esc(calendar.key)}" ${selected.has(calendar.key)?'checked':''}><span class="google-calendar-dot calendar-dot-${calendar.key}" aria-hidden="true"></span>${esc(label(calendar.key))}</label>`).join('')}</div><div class="google-calendar-links">${allUrl ? link(allUrl,'Google Calendar','text-link') : ''}${Object.entries(feedPaths).map(([key,path])=>link(path,esc(label(key))+' ICS','text-link')).join('')}</div></div><div class="google-calendar-frame"><iframe title="${esc(label('title'))}" ${url ? `src="${esc(url)}"` : 'hidden'} data-google-calendar-embed data-public-calendar-embed loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe><p class="google-calendar-empty" ${url?'hidden':''} role="status">${esc(label(calendars.length?'empty':'unavailable'))}</p></div></div>`;
}
