...
首頁 `/` 保留原站的組合順序：**大型限動預覽 → Google Calendar 官方嵌入 → 貼文河道**，整頁可向下瀏覽。`/post/` 則是獨立的完整動態頁，桌面使用佔滿可用視窗的多欄河道；首頁與動態頁有各自用途。
